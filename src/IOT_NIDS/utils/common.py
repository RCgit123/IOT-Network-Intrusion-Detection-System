...
def read_csv(path_to_csv: Path) -> pd.DataFrame:
    ...
    try:
        ...
        logger.info(f"CSV file: {path_to_csv} loaded successfully")
        return df
...
@ensure_annotations
def read_parquet(path_to_parquet: Path) -> pd.DataFrame:
    """Reads a Parquet file and returns the content as a Pandas DataFrame.

    Args:
        path_to_parquet (Path): Path to the Parquet file.
    ...
    try:
        ...
        logger.info(f"Parquet file: {path_to_parquet} loaded successfully")
        return df
    except pd.errors.EmptyDataError:
...
# @ensure_annotations
# def save_parquet(data: pd.DataFrame, path: Path):
